...
from models.retriever_MedCPT import retrieve_documents
from langchain.prompts import PromptTemplate

def generate_answer(query):
    # Retrieve documents: top 3 from each dataset
    pmc_docs, pubmed_docs = retrieve_documents(query, top_k=6)
    
    # Combine retrieved documents into a single context
    context = "\n".join([
        f"● {doc['title']}: {doc['abstract'][:500]}..." for doc in pmc_docs + pubmed_docs
    ])[:6000]  # Limit context length
